...
class Parse extends CI_Parser {
    ...
    public function __construct()
    {
        ...
        $this->CI->load->library('parser/smarty');
    ...
    */
    ...
    public function set_theme($name)
    {
        ...
        $functions_file = $this->CI->config->item('smarty.theme_path') . '/_plugins/inc.plugins.php';

 	$functions_file2 = $this->CI->config->item('smarty.theme_path') . $this->_theme_name .'/_plugins/inc.plugins.php';
 
        // Incase we have a theme in the application directory
        $functions_file3 = APPPATH.'/plugins/inc.plugins.php';
     ...
     * @return string
     */
    public function display($template, $data = array(), $return = FALSE, $caching = TRUE, $theme = '')
    {
    	...
    	$template_string = $this->CI->smarty->display('eval:'.$template);
    ...
    protected function _find_view($file)
    {
         ...
         if ( $current_module !== $this->_module )
        {
            ...
            $new_locations = array(
            	...
            	$this->CI->config->item('smarty.theme_path') . $this->_theme_name . '/' . $current_module .'/',
            	$this->CI->config->item('smarty.theme_path') . $this->_theme_name . '/',
            	$this->CI->config->item('smarty.theme_path') .  '/',
		$this->CI->config->item('smarty.theme_path').'/' . $this->_theme_name .'/',
            	APPPATH.'modules/' . $current_module .'/views/',
            	APPPATH.'modules/' . $current_module .'/themes/'.$this->_theme_name.'/',
            	APPPATH.'modules/' . $current_module .'/themes/',
            	APPPATH.'modules/' . $current_module .'/themes/default/'
            );
    ...
    */
    ...
    protected function _update_theme_paths()
    {
        ...
        $this->_template_locations = array(
            ...
            APPPATH.'modules/' . $this->_module  .'/themes/'.$this->_theme_name.'/',
            APPPATH.'modules/' . $this->_module  .'/themes/',
            APPPATH.'modules/' . $this->_module  .'/themes/default/'
        );
